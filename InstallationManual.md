...
・ [Entity_Type]、[Entity_ID]を決定し、
src/delivery_robot/param/ delivery_navi_node_tb3_01.yamlに反映します。
...
####（８）プログラムのリリースとビルド
・src/delivery_robotを[catkin_workspace]/src配下にリリースして下さい。
・devel/include/delivery_robotを[catkin_workspace]/devel/include配下にリリースして下さい。
・リリース後、以下コマンドを入力し、プログラムをビルドします。
```bash
$ cd ~/catkin_ws
$ catkin_make
```
...
　・ [Entity_Type]、[Entity_ID]を決定し、
src/delivery_robot/param/ delivery_robot_node_tb3_01.yamlに反映します。
...
####（７）WPの座標確認
・ナビゲーションを実施するに当たり、地図内の目標ポイント（WP）の座標をロボットに通知する必要があります。以下に会津大学様施設で実証実験を行った際に使用した地図を参考に示します。以下の図に示すアルファベットがWayPoint（目的地）を示しており、WayPointの座標を一覧化しておきます。地図の原点は、地図作成開始時点のロボット位置が原点となります。
　
【WayPointを明示した地図】
...
 
・WayPointの座標調査は、Navigation_Stackを起動した状態で行います。
　起動したrvizにて以下の操作を行います。
①rviz画面上部の「2D Pose Estimate」ボタンを押下する。
![image](https://github.com/jadsys/DeliveryRobotNode/wiki/images/08.png)

②地図上で、座標を知りたいポイント上で左クリック（押したままを維持）し、緑色の矢印が出力されたら、矢印を任意の方角に向け、左クリックを離す。
![image](https://github.com/jadsys/DeliveryRobotNode/wiki/images/09.png)
 
③Rvizを起動したターミナル上に、Informationとして「Setting Pose：[x] [y] [θ]」が出力されるため、こちらの座標をメモする。
![image](https://github.com/jadsys/DeliveryRobotNode/wiki/images/10.png)

####（８）移動指示用ノードプログラム修正
・移動指示用ノードプログラムには、会津大学様施設での実証実験用に取得した地図を元に参考経路をハードコーディングしております。（７）で整理したWayPoint一覧を元に同プログラムを修正して下さい。該当ソースは以下になります。
　
　【ソース名】
　　ディレクトリパス：~/[catkin_ws]/src/delivery_robot/src
　　ソース名　　　　：edge_node_beta.cpp
　
以下にプログラム起動イメージを示します。第1引数に[Entity_ID]、第2引数に[Entity_Type]、第3引数に経路パターンを指定します。上記のプログラム内のswitch処理内で、実証実験で使用した経路パターン「113」,「114」を記述しておりますので、こちらの座標を修正して下さい。
